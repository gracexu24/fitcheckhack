...
function submitForm(){
  ...
  city =  loc.substring(0, str.indexOf(","));
  state=  loc.substring(str.indexOf(",")+1);
...
fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city},${state}&appid=${apiKey}`)
   .then(response => response.json())
   .then(data => {
     console.log(`Current temperature in ${city}. ${state} is: ${data.main.temp} Kelvin. The weather is ${data.weather[0].description} `);
   })
   .catch(error => console.error(error));